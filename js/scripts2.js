...
var folderID;
    ...
    $('.fullhtml').click(function(e) {
			...
			if (isPopup == true) {
				...
				$.get($data, function(data) {
					$.get("00-Includes/controlsmap.php",{module:folder})
						.done(function(controls){

						//$('#container').append(data);
						$('#container').append(
						'<div class="contentarea_container blockme '+folder+'" data-module="'+folder+'">'+data+controls+'</div>'
						);

					console.log(folder+" outside of");
					if (folder == "header-600-wide" || folder == "headline") {
					console.log(folder+" inside of");
					...
					switch ($tmp) {
						case "tttd":
							...
							$(".header-600-wide").find(".maintitle").text(headline);
    ...
    $(document).delegate('.moduleform', 'submit', function(event) {
        ...
        formData = $(this).serialize();
				if($(this).attr("id")=="save_module"){


				var $theCode = $("body").find(".saving").html();
				var $moduleName = $(this).find('.save_module').val();
                ...
                var $data = $(data).appendTo('#container');
								$.get("00-Includes/controlsmap.php",{module:folder})
									.done(function(controls){
										...
										//$('#container').append(data);
									//	$('#container').append(
										//'<div class="contentarea_container blockme" data-module="'+folder+'">'+data+controls+'</div>'
										//);

										$data.filter('.contentarea').wrap('<div class="contentarea_container pasted active" data-module="freeform"></div>');
										$('.contentarea_container.pasted.active').append(controls);
										$('.contentarea_container.pasted.active').removeClass("active");
